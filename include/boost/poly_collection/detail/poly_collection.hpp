...
{
  template<typename T>
  static const std::type_info& sub_typeid(const T& x)
    {return Model::sub_typeid(x);}
  template<typename...>
...
public:
    ...
    const_local_base_iterator pos,InputIterator first,InputIterator last)
  {
    ...
    for(;first!=last;++first){
      BOOST_ASSERT(pos.type_index()==sub_typeid(*first));
...
private:
  ...
  const_segment_map_iterator get_map_iterator_for(const T& x)
  {
    ...
    else if(sub_typeid(x)!=typeid(T))throw unregistered_type{sub_typeid(x)};
    ...
    const T& x,const segment_type& seg)
  {
    ...
    if(it!=map.end())return it;
      ...
      {sub_typeid(x),segment_type::make_from_prototype(seg)}).first;
  }
